...
void convert(CSR<T>& dst, const CSR<T>& src, unsigned int padding)
{
	dst.alloc(src.rows + padding, src.cols, src.nnz + 8 * padding);
	dst.rows = src.rows; dst.nnz = src.nnz; dst.cols = src.cols;
	memcpy(dst.data, src.data, dst.nnz * sizeof(T));
	memcpy(dst.col_ids, src.col_ids, dst.nnz * sizeof(unsigned int));
	memcpy(dst.row_offsets, src.row_offsets, (dst.rows + 1) * sizeof(unsigned int));
}
